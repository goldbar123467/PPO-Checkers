...
1. Commit the source-audited statistical caveats in the machine-report contract.
2. Produce and resume-validate the final report from that exact clean source SHA.
3. Write the sourced conclusion, run the consolidated Phase 5 gate, and update phase state.
...
Commit the green source-audit correction, archive the superseded output, then execute
`.venv-train/bin/python scripts/evaluate_baselines.py --progress-log logs/gates/phase-5-baseline-run.txt`
from the final clean commit. Resume once, compare exact hashes, and write the sourced conclusion.
