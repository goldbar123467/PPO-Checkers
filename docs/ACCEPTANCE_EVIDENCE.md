...
| 6 — RL core | §§7–9, §12.6–12.7, §14 Phase 6; PPO/GAE primary papers | T1–T8, dtype masking, CPU/GPU determinism. | IN PROGRESS: T1–T6, T8, masking, D1–D3 GREEN with focused 100% coverage; T7 and consolidated gate remain. |
...
| E9 | T1–T8, including hand-computed signed GAE. | Passing tests with derivation fixtures. | PARTIAL: T1–T6 and T8 pass; only T7 and the consolidated Gate 6 replay remain. |
| E10 | D2/D3 and exact mid-sequence resume. | Determinism/resume transcripts and metadata. | PROVEN for implementation: D2 ten-update CPU bitwise equality, D3 same-stack RTX action/tolerance checks, and exact scalar/vector mid-sequence restore pass with stack metadata recorded. Final README instructions remain a deliverable audit item. |
